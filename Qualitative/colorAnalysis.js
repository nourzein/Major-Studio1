...
//read files
fs.readFile("./codebeautify.json", "utf8", (err, data) => {
  ...
  imagesArray.forEach(async imageObj => {
    ...
    if (counter === imagesArray.length) {
      let smallimage = imageObj.image.replace(/original/g, "web-large");
      fs.writeFile(
        "finalImages.json",
...
function getDominantColor(imageObj) {
  const fileName = "./downloads/" + imageObj.image.replace(/^.*\//, ""); //replace everything before the / with nothing
