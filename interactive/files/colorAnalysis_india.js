...
//read files
fs.readFile("./codebeautify_india.json", "utf8", (err, data) => {
...
function getDominantColor(imageObj) {
  const fileName = "./downloads_india/" + imageObj.image.replace(/^.*\//, ""); //replace everything before the / with nothing
  // const fileName = "./downloads/DT1953.jpg"; //replace everything before the / with nothing

  return ColorThief.getPalette(fileName, 5)
    .then(color => {
